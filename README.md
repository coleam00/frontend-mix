...
- **Sonnet** explores the repo and scopes the spec into a context handoff.
...
- `.claude/skills/frontend-mix-*` (eight skills, one per phase) for running the chain by hand.
...
For Claude Code (all eight frontend-mix skills + the archon skill):
...
1. `/frontend-mix-explore <spec.md path or "description">` (Claude Code, Sonnet)
2. `/frontend-mix-plan <context.md path>` (Claude Code, Opus)
3. `/skill:frontend-mix-design <plan path>` (Pi, Gemini 3.5 Flash)
4. `/frontend-mix-integrate <plan path> <ui-summary path>` (Claude Code, Opus)
5. `/frontend-mix-validate <integration-summary path>` (Claude Code, Sonnet)
6. `/frontend-mix-fix-validation <validation-issues path> <plan path>` (Claude Code, Opus, only if step 5 flagged failures)
7. `/frontend-mix-smoke <integration-summary path> <validation-summary or resolution-summary path>` (Claude Code, Sonnet)
8. `/frontend-mix-deploy <plan path> <validation-summary or resolution-summary path>` (Claude Code, Opus)

<details>
<summary><b>Detailed walkthrough</b> (click to expand)</summary>

### 1. Explore - Claude Code with Sonnet

The explore step accepts **either** an absolute path to a spec markdown **or** a free-form description of what you want to build. Both work; whichever is more convenient. This is cheap context-gathering, so run it on Sonnet.

With a spec file:

```
/frontend-mix-explore C:/path/to/spec.md
```

With a plain-text description:

```
/frontend-mix-explore "A landing page + sign-in + dashboard for an AI image-gen SaaS, Clerk for auth"
```

The skill picks a run-name slug from your input (e.g. `acme-saas-landing`), inspects the repo, and writes:

```
.claude/artifacts/acme-saas-landing-context.md
```

Note that path. The plan step needs it.

### 2. Plan - Claude Code with Opus 4.8

Switch the model to Opus and hand the plan step the context path from step 1. It reads the context (and the spec the context points to) and writes the three-section plan - no re-exploration:

```
/frontend-mix-plan C:/.../acme-saas-landing-context.md
```
...
### 3. Design - Pi with Gemini 3.5 Flash
...
### 4. Integrate - Claude Code with Opus 4.8
...
### 5. Validate - Claude Code with Sonnet
...
### 6. Fix validation - Claude Code with Opus 4.8 (only if step 5 wrote `validation-issues.md`)
...
### 7. Smoke - Claude Code with Sonnet
...
### 8. Deploy - Claude Code with Opus 4.8
